...
class SalesReportTable extends Component {
    ...
    filerRecords = (_id) => {
        ...
        let new_records = temp_records.filter(record=>{
            if(record._id.includes(_id) || record.customerName.toLowerCase().includes(_id.toLowerCase()))
              return record; 
        });
    ...
    jsonToCsv = (data) => {
        let new_array = [];
        for(let row of data){
            new_array.push([row._id,row.customerName, row.totalAmount, row.invoiceDate]);
        }
        return new_array;
    }

    render() { 
        const { salesRecordIDFiler } = this.state;
        //filtering records using the invoice id
        const filtered_records = this.filerRecords(salesRecordIDFiler);
            ...
            <>
                <div className="col-lg-6">
                    ...
                    <input type="text" 
                        ...
                        name="salesRecordIDFiler"
                        onChange={this.handleInput}
                        value={salesRecordIDFiler}
                        placeholder="Filer using invoice ID"
                    />
                        ...
                        <tbody>
                            {
                                filtered_records.map(record=>(
                                    <tr className="table-row" onClick={()=>this.setState({currentSelectedReport: record})}>
                                        <td>{record._id}</td>
